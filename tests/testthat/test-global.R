test_that("global theme api works", {
  ...
  primary <- bs_sass("body{background:$primary;}", theme = bs_global_get())
...
test_that("Theme adding works as intended", {
  ...
  css <- bs_sass(".foo{color:$primary;}", theme = bs_global_get())
  ...
  css <- bs_sass(".foo{color:$primary;font-size:0}", theme = bs_global_get())
