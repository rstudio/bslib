...
# Sys.setenv(UPDATE_SCREENSHOT = TRUE)
...
in one of two ways:
you can provide a character value describing the theme, e.g. `theme = "primary"`;
or you can use `value_box_theme()` to create a custom theme.
We recommend you use the first approach (using named themes)
for most value boxes,
...
Note that `value_box_theme()` also takes the `theme` name, which can be helpful if you want to use a named theme and modify the default `bg` or `fg` colors of that theme.
Also note that these colors must be CSS colors, not Bootstrap theme or color names.
