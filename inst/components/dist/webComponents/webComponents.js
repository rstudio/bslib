...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // srcts/src/components/statusButton.ts
  var BslibStatusButton = class extends LightElement {
    constructor() {
      super();
      this.label = "";
      this.icon = "";
      ...
      this.pending = { disable: true };
      this.success = { disable: false };
      this.failure = { disable: false };
      this.state = "ready";
      this.buttonElement = this.children[0];
      this.iconElement = this.querySelector(".bslib-status-icon");
      this.labelElement = this.querySelector(
        ".bslib-status-label"
      );
      this.helpElement = this.querySelector(".bslib-status-help");
    ...
    _handleGenericStateChange(state) {
      this._cancelTimer();
      ...
      if (state.delay) {
        this.timer = setTimeout(() => this.state = "ready", state.delay * 1e3);
      }
      if (state.spinner) {
    ...
    _handleDisabledState(disable = true) {
      if (disable) {
    ...
    _setClass(cls) {
      ...
      cls.split(" ").forEach((c3) => this.buttonElement.classList.add(c3));
    }
    _cancelTimer() {
      if (this.timer)
        clearTimeout(this.timer);
  ...
  BslibStatusButton.tagName = "bslib-status-button";
  BslibStatusButton.isShinyInput = true;
  __decorateClass([
    n5({ type: String })
  ], BslibStatusButton.prototype, "state", 2);
  if (window.Shiny) {
    ...
    window.Shiny.addCustomMessageHandler(
      ...
      function(message) {
        ...
        if (!el)
          return;
        el = el.parentElement;
        if (!(el instanceof BslibStatusButton))
          return;
        if (message.type === "state") {
          if (el.state === message.data) {
            el.state = "";
          }
          el.state = message.data;
        }
      }
    );
  }
