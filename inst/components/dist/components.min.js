...
`+s.value.substring(t),s.selectionStart=s.selectionEnd=e+1,s.dispatchEvent(new Event("input",{bubbles:!0}))}var w,Z,D,Q=f(()=>{"use strict";S();w="textSubmitInputBinding",Z=new IntersectionObserver(s=>{s.forEach(e=>{e.isIntersecting&&B(e.target)})}),D=class extends b{find(e){return $(e).find(".bslib-input-submit-textarea textarea")}initialize(e){J(e),B(e),re(e)}getValue(e){return $(e).data("val")}setValue(e,t){e.value=t}subscribe(e,t){function i(){$(e).data("val",e.value),e.value="",e.dispatchEvent(new Event("input",{bubbles:!0})),t("event")}let n=R(e);n.classList.contains("shiny-bound-input")?$(n).on(`shiny:inputchanged.${w}`,i):$(n).on(`click.${w}`,i),$(e).on(`input.${w}`,function(){J(e),B(e)}),$(e).on(`keydown.${w}`,function(r){if(r.key!=="Enter")return;if(!e.value){r.preventDefault();return}if(r.shiftKey)return;if(r.altKey){r.preventDefault(),ae(e);return}let a=e.hasAttribute("data-needs-modifier");if(!a){r.preventDefault(),n.click();return}let d=r.ctrlKey||r.metaKey;if(a&&d){r.preventDefault(),n.click();return}}),Z.observe(e)}unsubscribe(e){$(e).off(`.${w}`);let t=e.nextElementSibling;$(t).off(`.${w}`),Z.unobserve(e)}receiveMessage(e,t){return u(this,null,function*(){let i=e.value;if(t.value!==void 0&&(e.value=t.value,e.dispatchEvent(new Event("input",{bubbles:!0}))),t.placeholder!==void 0&&(e.placeholder=t.placeholder),t.label!==void 0){let n=$(e).closest(".shiny-input-container").find("label");yield P(t.label,n)}t.submit&&(R(e).click(),e.value=i),t.focus&&e.focus()})}};v(D,"submit-text-area")});function ee(s){if(window.Shiny)for(let[e,t]of Object.entries(s))window.Shiny.addCustomMessageHandler(e,t)}var te=f(()=>{"use strict"});var le=ne(se=>{V();G();j();Y();Q();S();te();var oe={"bslib.toggle-input-binary":s=>u(se,null,function*(){let e=document.getElementById(s.id);e||console.warn("[bslib.toggle-input-binary] No element found",s);let t=$(e).data("shiny-input-binding");if(!(t instanceof b)){console.warn("[bslib.toggle-input-binary] No input binding found",s);return}let i=s.value;typeof i=="undefined"&&(i=!t.getValue(e)),yield t.receiveMessage(e,{value:i})})};window.Shiny&&ee(oe);function ie(){let s=document.createElement("div");s.innerHTML=`
  ...
  </svg>`,document.body.appendChild(s.children[0])}document.readyState==="complete"?ie():document.addEventListener("DOMContentLoaded",ie)});le();})();
