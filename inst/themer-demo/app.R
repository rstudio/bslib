...
rounded <- isTRUE(
  as.logical(bs_get_variables(theme %||% bslib::bs_theme(), "enable-rounded"))
)
pill <- function(...) {
...
tab <- function(...) {
  shiny::tabPanel(
    ...
    class = if (rounded) "rounded-bottom"
  )
}
    ...
    nav_panel(
      ...
      tabsetPanel(
        ...
        pill(
          ...
          tags$p(
            "Here are some",
            tags$code("actionButton()"),
            "s demonstrating different theme (i.e., accent) colors"
          ),
          layout_buttons(
            # Regular <div> if IS_LEGACY, otherwise layout_columns
            actionButton(
              "primary",
              "Primary",
              icon("product-hunt"),
            ...
            actionButton(
              ...
              "Danger",
              icon(
                if (packageVersion("fontawesome") >= "0.3")
                  "triangle-exclamation" else "exclamation-triangle"
              ),
