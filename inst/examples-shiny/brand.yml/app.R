...
if (requireNamespace("thematic", quietly = TRUE)) {
  base_font <- brand.yml::brand_pluck(brand, "typography", "base", "family")
...
if (brand.yml::brand_has(brand, "typography", "fonts")) {
  tryCatch(
    {
...
server <- function(input, output, session) {
  ...
  output$brand_name <- renderUI({
    brand_name <-
      brand.yml::brand_pluck(brand_yml(), "meta", "name", "short") %||%
      brand.yml::brand_pluck(brand_yml(), "meta", "name")
  ...
  output$brand_icon <- renderUI({
    ...
    logo <-
      brand.yml::brand_pluck(brand, "logo", "small") %||%
      brand.yml::brand_pluck(brand, "logo")
