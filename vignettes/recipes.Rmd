...
> **DISCLAIMER:** This document demonstrates what's possible _today_ with **bootstraplib** theming and is still a work in progress. It should become more complete and user friendly in the coming months.
...
[A future version of **shiny**](https://github.com/rstudio/shiny/pull/2740), `renderPlot()` will be able to use it's foreground/background color to automatically apply an intelligent default theme to **ggplot2**, **lattice**, and **base** graphics. That means, even if you've only changed `$body-bg` and `$body-color`, your plots should inherit them and look consistent with the rest of your app's styling.
