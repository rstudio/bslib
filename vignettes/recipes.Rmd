...
We'll walk through theming with tabs (i.e., `tabsetPanel()`) and pills (i.e., `navlistPanel()`) in **shiny**, but it's worth noting that the same ideas apply to [**rmarkdown**'s tabbed sections](https://bookdown.org/yihui/rmarkdown/html-document.html#tabbed-sections) as well.


### Tabs

By default, `tabPanel()` doesn't include any padding or border around its content, which wasn't necessarily the best design choice. Thankfully, similar to `actionButton()`, we can add [utility](https://getbootstrap.com/docs/4.4/utilities/spacing/) [classes](https://getbootstrap.com/docs/4.4/utilities/spacing/) to the panel's content container by add padding (`p-*`) and `rounded` `border`s. That means, at the top of the app, we could do something like this to add padding and rounded borders to all the tabsets (In practice, you might want to add these to individuals `tabPanel()`s instead of overwriting the function).
...
```{r, echo = FALSE}
knitr::include_graphics("https://i.imgur.com/iQubNd3.png")
```

Similarly, if we set `type = "pills"` in `tabsetPanel()`, then you'd probably want slightly different borders:
