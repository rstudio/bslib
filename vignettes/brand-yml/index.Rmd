...
::: lead
Shiny for R now supports [brand.yml], a simple, unified theming experience through a single YAML file.
Use [brand.yml] in your Shiny apps, Quarto projects, and more.
:::
...
If the app [uses `theme = bs_theme()`](../any-project/), any of the [`bslib::page_*()` functions](../dashboards/), or [bslib theming under-the-hood](../theming/),
bslib will automatically find and apply the branded theming from a `_brand.yml` file in the current or parent project directories.
...
```r
shiny::runExample("brand", package = "bslib")
```
