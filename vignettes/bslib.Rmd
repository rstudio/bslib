...
As we learned in [Basic usage](https://rstudio.github.io/bslib/#usage), any usage of `bslib` with Shiny and R Markdown is powered by `bs_theme()`. Here you can pick a major Bootstrap [version](#versions), choose a pre-packaged [Bootswatch](#versions) theme, and/or [customize](#custom) various theming defaults. `bslib` even provides a interactive widget (`bs_themer()`) to change some of `bs_theme()`'s options [real-time](#real-time). 
