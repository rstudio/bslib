...
To create a sidebar layout, provide a `sidebar()` to the `sidebar` argument to `layout_sidebar()` or any of the `navs_` or `page_` functions that support sidebars.

:::: {.row}
::: {.col-sm-12 .col-lg-6}
### Shared Sidebar {#intro-shared-sidebar}

When `sidebar` is an argument to a function containing multiple pages or tabs, it creates a **[shared sidebar](#shared-sidebar)**, meaning the same sidebar is visible on every page/tab.

Provide a `sidebar()` to `page_navbar()` to recreate the classic [shinydashboard](http://rstudio.github.io/shinydashboard/) look with a global sidebar [shared across several pages](#navbar-pages).

You can also add a [shared sidebar to a card with tabs or pills](#card-tabs-shared-sidebar) created with `navs_tab_card()` or `navs_pill_card()`.
:::
...
Use `layout_sidebar()` whenever you want a **[local sidebar](#local-sidebar)** adjacent to a content panel.
`layout_sidebar()` creates a bordered area by default, making it useful anywhere in your interface.

It also fits nicely into [card components](#cards) and [nav pages or tabs](#navbar-pages) allowing you to add sidebar(s) within individual pages or tabs.
:::
...
Use global sidebars sparingly and only when their controls affect all the outputs on the page.

Note that while we talk about shared and local sidebars in this article, those are properties of the interface design.
Controls placed in a sidebar aren't inherently limited to affecting only the outputs next to the sidebar.
