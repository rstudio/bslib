...
Depending on the function, the sidebar is either **global**  -- where the same sidebar is used for all pages or tabs -- or it is **local** to a particular page or tab.
...
describe_layout_function(
  ...
  "A shared sidebar for multiple tabs within a card.",
  card_body_fill(
    class = "p-2",
    height = 200,
    navs_tab_card(
...
describe_layout_function(
  "card_sidebar()",
  "A sidebar layout within a card body.",
  card_body_fill(
    class = "p-2",
    height = 200,
    card(
...
When choosing which layout to use, keep in mind that a good usability rule of thumb is to group sidebar controls as closely as possible with the contents that the sidebar affects.
If you have multiple groups of outputs you want to show on the same page, you may want to put several card-based sidebar layouts on a given page.
...
To create a sidebar layout that's specific to a page, place a `card_sidebar()` inside a tab (i.e., `nav()`) container (similar to how you would for [a `card()` without tabs](#cards)).
