...
That is, for a singular sidebar (i.e., a sidebar that is shown on every tab), provide a `sidebar()` object to the `sidebar` argument:
...
navs_tab_card(
  ...
  nav("Tab 1 (Price)", plot_price),
  nav("Tab 2 (Carat)", plot_carat)
)
```


Or, if you want only particular tabs to contain a sidebar, provide a `card_sidebar()` to relevant `nav()` tabs:
...
page_navbar(
  title = "Singular sidebar",
  sidebar = sidebar(cut, clarity),
  nav("Page 1 (Price)", plot_price),
  nav("Page 2 (Carat)", plot_carat)
)
```
...
page_navbar(
  ...
  sidebar = sidebar(cut, clarity),
  fillable = "Price",
  nav("Page 1 (Price)", plot_price, value = "Price"),
  nav("Page 2 (Carat)", plot_carat)
)
```
...
`sidebar()` (and `layout_sidebar()`) has a handful of named arguments for customizing sidebar `position`, the `bg` color, whether sidebar is `open` by default, and more.
...
All the arguments to `layout_sidebar()` apply to the main content area. Thus, you could pass a [utility class like `p-0`](https://getbootstrap.com/docs/5.3/utilities/spacing/) to remove/reduce/add to the default padding. 
...
Inside a Shiny context, the `open` state of a `sidebar()` can programmatically updated and/or queried through an input value. Just provide an `id` and refer to that id in the server-side code. Here's an example of using an `actionButton()` as an alternative way of toggling the open/closed state of the sidebar:
...
server <- function(input, output) {
  observeEvent(input$toggle, {
    if (input$sidebar) sidebar_close("sidebar") else sidebar_open("sidebar")
  })
}
