...
3. `navset_card_tab()` (or `navset_card_pill()`)
...
page_navbar(
  ...
  nav_panel("Page 1", plot_price, plot_carat),
  nav_panel("Page 2", plot_depth)
)
```
...
To create a sidebar layout that's specific to a page, place a `layout_sidebar()` inside a page (i.e., `nav_panel()`) container. In addition to allowing pages to not have a sidebar, this approach also makes it possible to make a group of related inputs/outputs visually distinct from other (unrelated) inputs/outputs.
...
page_navbar(
  ...
  nav_panel("Page 2", "Something else")
)
```
...
It's also possible to embed a sidebar layout within a `card()`s that has multiple tabs/pills via `navset_card_tab()`/`navset_card_pill()`. And, similar to `page_navbar()`, a `sidebar()` can either be passed as an argument (i.e., global sidebar) or to a particular tab/pill (i.e., local sidebar).

#### Global sidebar

To show the same `sidebar()` on every tab/pill (i.e., create a "global" sidebar), pass it to the `sidebar` argument of `navset_card_tab()` (or `navset_card_pill()`).

Remember that, just like `layout_sidebar()` and `card()`, `navset_card_tab()` sizes itself according to it's contents (by default), so consider setting a `height` to keep a fixed height across all tabs. And, to allow the contents to grow/shrink to fit the card container, you'll want to wrap the contents of each tab with `card_body_fill()` (`wrapper = card_body_fill` is a convenient way to do this for all tabs).
...
navset_card_tab(
  ...
  nav_panel("Tab 1", plot_price, plot_carat),
  nav_panel("Tab 2", plot_depth)
)
```
...
dynamic <- navset_card_tab(
  ...
  id = "nav",
  nav_panel("Page 1", plot_price),
  nav_panel("Page 2", plot_carat, plot_depth)
)
...
To create a sidebar layout that’s specific to a page, place a `layout_sidebar()` inside a tab (i.e., `nav_panel()`) container (similar to how you would for [a `card()` without tabs](#cards)).
