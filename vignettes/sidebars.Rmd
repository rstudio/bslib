...
If you have a lot of content in a `layout_sidebar()`, one option is to specify a `height` for the container, making its contents scrollable.
...
If, instead of scrolling contents (the default), you want to allow those contents to resize themselves to that height, also set `fillable = TRUE`:
