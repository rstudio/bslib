...
`layout_sidebar()` lets you create a sidebar layout for individual pages when your inputs _aren't shared_ across the outputs on multiple pages.
Place a `layout_sidebar()` inside a `nav()` container to create a local sidebar.
In the example below, we used a sidebar layout for Page 1's plot's filter controls without including these inputs next to the unrelated map on Page 2.
...
In fact, there's content on Page 1 below the container with the sidebar -- and that content could even include other containers with local sidebar layouts.
In the next section, we'll show you how to create a single full-page sidebar for just one page.

### Fillable sidebar layouts
...
When a container is fillable, any _fill items_ in the container will grow or shrink to fill all available space in the container.

Let's return to the shared sidebar example.
If we set `fillable = TRUE`, any fill items in each page will grow or shrink to fill the main content area.
...
Like plotly plots, `layout_sidebar()` creates a fill item by default, so to create a single-page sidebar layout that fills the browser window we only need to make the relevant page fillable.
...
We'll take the latter approach in the following example.
...
Then, the plotly plots (which are fill items) will fill the main content area of the sidebar layout (which is a fillable container _and_ a fill item) which will fill the page (since it's a fillable container).
...
Try grabbing the lower right corner and resizing the example output to see how the plots grow and shrink to fill their container's height.
