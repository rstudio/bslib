...
Both `sidebar()` and `layout_sidebar()` have named arguments which, as we've already seen, are helpful for customizing the styling and behavior of both the sidebar and main content areas. For example, the previous example demonstrates `sidebar()` options such as `position = "right"` and `open = FALSE`
...
tagAppendAttributes(side, style = css("--bs-border-color" = "darkgray"))
```
