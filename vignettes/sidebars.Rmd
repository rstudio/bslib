...
In the [Getting Started with dashboards](dashboards.html) article, we covered "page-level" sidebar layouts via the `page_sidebar()` and `page_navbar()` functions. In this article, we'll explore the full range of sidebar layouts available in bslib.
:::
...
::: {.col-sm-12 .col-lg-6}
In the [Getting Started with dashboards](dashboards.html) article, we saw how `page_sidebar()` yields a sidebar layout that fills the page. Underneath the hood, `page_sidebar()` is just a simple wrapper around `page_fillable()` and `layout_sidebar()`. Understanding this unlocks the potential to have (any number of) sidebar layouts within a filling layout. 
:::
...
As we covered in [Getting Started with dashboards](dashboards.html), the `sidebar` argument of `page_navbar()` puts a sidebar _on each page_ that fills the window. However, sometimes it's better that only particular pages have such a sidebar layout. To acheive this, just provide a `layout_sidebar()` as a "root" element of a `fillable` page.
