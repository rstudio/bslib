...
The card-level sidebar even works with the `full_screen = TRUE` option, in which case you probably also want to set `fillable = TRUE` in `layout_sidebar()` so that its contents will grow or shrink to fill the card container.
...
And, similar to `page_navbar()`, a shared `sidebar()` can either be passed as an argument to `sidebar` or to created as a local sidebar for a particular ta b or pill with `layout_sidebar()`.
