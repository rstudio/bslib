...
Be wary that `page_navbar()`'s `sidebar` argument puts the same sidebar on every page, which may or may not be desirable. In the [sidebar article](../sidebars), you'll learn some strategies for keeping the sidebar's contents close to the stuff they effect. Specifically, that article covers how to: (1) [hide/show `sidebar` contents on particular pages](../sidebars#conditional-contents) (2) [put a particular `sidebar()` with a given page](../sidebars#multi-page-example), and even (3) [put sidebar layouts inside of a `card()`](../sidebars#multi-page-example).
