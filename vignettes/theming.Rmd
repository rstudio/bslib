...
If you don't have a particular app or document in mind, you can also use `bs_theme_preview()` to create a demo Shiny app with the theming widget already overlayed ([see here](https://testing-apps.shinyapps.io/themer-demo) for a hosted version):
...
```{r, echo = FALSE}
knitr::include_graphics("images/custom-dark-mode.png")
```
...
Utility classes are primarily helpful for styling at the component (rather than the page) level, and is particularly handy for things like spacing, border, colors, and more. See the article on [Utility Classes](utility-classes.html) for some useful examples specific to Shiny and R Markdown.
