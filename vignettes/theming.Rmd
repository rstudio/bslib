...
`session$setCurrentTheme()` "just works" for existing themable components (e.g., "core" Shiny UI), but in order for it to work with custom components, the component's CSS must come from a "dynamic" HTML dependency (i.e., `bs_dependency_hook()`). To learn more, see the section on themable custom components.
...
person <- function(name, title, company) {
  div(
    ...
    div(class = "title", title),
    div(class = "company", company),
    bs_dependency_hook(person_dependency)
  )
}
```
