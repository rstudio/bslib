...
This article focuses on using `bslib` to create the user interface (UI) for [Shiny](https://shiny.posit.co/) dashboards. In the [next getting started article](any-project.html), we'll also cover how these dashboard [components](#components), [layout helpers](#layouts), etc. be also be used in isolation inside of any Bootstrap project (e.g., R Markdown, `{pkgdown}`, etc), not just Shiny dashboards.
...
```{r, out.height = 800, out.width = 1200}
ui <- page_sidebar(
  title = "Penguins dashboard",
  sidebar = color_by,
...
```{r, out.height = 800, out.width = 1600}
ui <- page_sidebar(
  title = "Penguins dashboard",
  sidebar = color_by,
  layout_columns(
...
Be wary that `page_navbar()`'s `sidebar` argument puts the same sidebar on every page, which may or may not be desirable. In the [sidebar article](sidebar.html), you'll learn some strategies for keeping the sidebar's contents close to the stuff they effect. Specifically, that article covers how to: (1) [hide/show `sidebar` contents on particular pages](sidebars.html#conditional-contents) (2) [put a particular `sidebar()` with a given page](sidebars.html#multi-page-example), and even (3) [put sidebar layouts inside of a `card()`](sidebars.html#multi-page-example).



### Multi-panel

Multiple `nav_panel()`s don't necessarily need to go directly into `page_navbar()`. They can also be consumed by any `navset_*()` component, including `navset_card_tab()`.
...
Filling layout, in general, may not be desirable, especially when there is a lot of output to display on a single page. In this case, consider setting `fillable = FALSE` to prevent growing/shrinking altogether (in this case, outputs then 'fallback' to their default height which is usually 400px).

```{r, out.height = 700, out.width = 1000}
ui <- page_sidebar(
  title = "Penguins dashboard",
  sidebar = color_by,
  fillable = FALSE,
...
```{r, out.height = 600, out.width = 400}
ui <- page_sidebar(
  title = "Penguins dashboard",
  sidebar = sidebar(color_by, open = TRUE),
...
A `value_box()` is a special kind of card designed for highlighting a value along with a title and a `showcase` placeholder (typically a `{bsicons}` icon). They are covered in more detail in the [value boxes](value-boxes.html) article, but here's a `{palmerpenguins}` example:

```{r, out.height = 900, out.width = 1200}
means <- colMeans(
...
We've already seen how the `sidebar` argument of `page_sidebar()` and `page_navbar()` yields a "page-level" sidebar layout, but they can also be made at the "component-level" via `layout_sidebar()`. This is especially useful for keeping input controls near the outputs they effect in more complex apps. `layout_sidebar()` also works inside of a `card()`, making it easy to add headers, footers, `full_screen` behavior, etc.

To learn more, visit the [cards](cards.html) and [sidebars](sidebars.html) articles.
...
In the getting started with [theming article](theming.html), you'll learn some general theming strategies that also translate to dashboards. That is, by passing a `bs_theme()` object to the `theme` argument of `page_sidebar()` (or `page_navbar()`), we can add a [Bootswatch](https://bootswatch.com/) theme, easily use [Google Fonts](https://fonts.google.com/), customize 100s of [theming variables](bs5-variables.html), and much more. Keep in mind, however, that `bs_theme()` only customizes CSS styles. So, if you want `plotOutput()`s to match the CSS, consider using the `{thematic}` package to translate the styles to `ggplot2::theme()` settings. When doing so, also be aware that "complete" `{ggplot2}` themes (e.g., `theme_bw()`) only work sensibly when setting them globally (i.e., `theme_set(theme_bw())`).

```{r, delay = 2, out.height=700}
ui <- page_sidebar(
  title = "Penguins dashboard",
  sidebar = color_by,
  theme = bs_theme(
