...
<!-- 
Maybe this needs to be reframed as "modifying themes" or "low-level" theming
-->
...
**bootstraplib** themes are built upon [sass::sass_layer()](https://rstudio.github.io/sass/articles/sass.html#layers)s, which provides a basis for packaging and combining multiple 'layers' or 'bundles' of Sass in a composable way. Layers, and therefore themes, have three main components: `defaults` (for overriding variable defaults), `declarations` (for declaring Sass functions/mixins), and styling `rules`. Indeed, when you initialize a theme with `bs_theme()`, the class and names of the theme reflect these core pieces of the **sass** layer:
...
```r
my_theme <- bs_add_defaults(my_theme, "input-bg" = "red")
sass::as_sass(my_theme$defaults)
```

```css
/* Sass */
$input-bg: red !default;
...
```r
my_theme <- bs_add_defaults(my_theme, "input-bg" = "green")
sass::as_sass(my_theme$defaults)
```

```css
/* Sass */
$input-bg: green !default;
$input-bg: red !default;
@import "/path/to/bootstrap/scss/_functions.scss";
@import "/path/to/bootstrap/scss/_variables.scss";
```
...
bs_sass(".foo { color: $primary; }", theme = my_theme)
```
...
For an example of (2), see this [person.scss](https://github.com/rstudio/bootstraplib/blob/master/inst/custom/person.scss) file, which defines rules for a custom `person` class, for displaying someone's name, title, and company. To include these rules with the bundle that `bs_dependencies()` generates, use `bs_add_rules()` to add them to a `bs_theme()` object.^[You can also compile these rules with `bs_sass()` which returns a CSS string. In that case, you'd have to manually [include the string as a `<style>` tag](http://rstudio.github.io/sass/articles/sass.html#shiny-string).]

```r
library(shiny)
# Define some custom HTML that will utilize Bootstrap Sass
...
# Include some CSS rules for person() in the Bootstrap CSS
my_theme <- bs_theme() %>%
  bs_add_rules(
    sass::sass_file(system.file("custom", "person.scss", package = "bootstraplib"))
  )
ui <- fluidPage(
  theme = my_theme,
...
Themes have three main components: `defaults` (for overriding variable defaults), `declarations` (for declaring Sass functions and mixins), and styling `rules`. `bs_add_layers()` may be used to add any of these components to the existing theme and `bs_add_defaults()` is specifically designed for setting variable `defaults`. Recall that when you add to an existing theme:
