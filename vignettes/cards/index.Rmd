...
Toolbars are compact containers for buttons, inputs, and other UI elements that work particularly well in card headers and footers. They provide a clean way to organize controls related to the card's content, allowing you to clearly show that a selection of inputs pertain to that particular card.

See the [toolbar reference documentation](../reference/toolbar.html) for more examples and run `shiny::runExample("toolbar", package = "bslib")` for a complete interactive demo.

::: {.row .mt-3}
::: col-md-6
```{r toolbar-card, eval=FALSE}
card(
  full_screen = TRUE,
  card_header(
    "Sales Data",
...
::: col-md-6
```{r ref.label="toolbar-card", echo=FALSE}
```
:::
:::
