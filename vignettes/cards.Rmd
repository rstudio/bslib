...
One major feature that bslib adds to Bootstrap cards is the ability to expand the card to a [full screen view](#responsive-sizing). Often this feature wants [output that resizes itself to fit its card container](#responsive-sizing). To do this today in the same way presented in this article, you'll need the development version of numerous packages:

```r
remotes::install_github(c("rstudio/bslib", "rstudio/shiny", "ramnathv/htmlwidgets", "ropensci/plotly", "rstudio/leaflet", "rstudio/DT"))
```

Furthermore, if you'd like to use [htmlwidgets](http://www.htmlwidgets.org/) other than `{plotly}`, `{leaflet}`, or `{DT}` with [responsize sizing](#responsive-sizing) in [Shiny](#dynamic-rendering-shiny), you may need to set `height = NULL` in it's output container (i.e., `widgetOutput("id", "height = NULL)`). 

Since this article is statically hosted (i.e., not powered by Shiny), it uses statically rendered [htmlwidgets](http://www.htmlwidgets.org/) like `{plotly}` and `{leaflet}`. Here's some code to create those widgets:
...
1. It doesn't require a defined height on the `card()` (or a parent of the card).
2. It allows it's direct children to grow and shrink when the `card()` does have a defined height.
:::
