...
Since this article is statically hosted (i.e., not powered by Shiny), it uses statically rendered [htmlwidgets](http://www.htmlwidgets.org/) like `{plotly}` and `{leaflet}` (but don't worry, `card()`s [work in Shiny equally as well](#shiny)). Here's some code to create those widgets:
...
::: row
::: col-md-6
Utility classes are really useful since they not only also help with spacing and alignment of stuff _within_ a `card_body()` (or `card_body_fillable()`), but more generally enable easy customization of colors, fonts, and more.
:::
