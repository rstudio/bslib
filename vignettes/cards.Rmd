...
When rendering htmlwidgets via Shiny, you'll also need `height` set to `NULL` (e.g., `leafletOutput(height = NULL)`), some dev version(s) of now do this by default:

```r
remotes::install_github(c("ropensci/plotly", "rstudio/leaflet", "rstudio/DT"))
```

The same goes for `plotOutput()` and `imageOutput()`:
