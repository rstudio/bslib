...
Cards work equally well [in Shiny](#shiny). In the examples below, replace `plotly_widget` with `plotly::plotlyOutput()` and `leaflet_widget` with `leaflet::leafletOutput()` to adapt them for Shiny server-rendered plots/maps.
:::
...
::: {.row .mt-5}
::: col-md-6
Most htmlwidgets (e.g., plotly, leaflet, etc) and some other Shiny output bindings (e.g, `shiny::plotOutput()`, `shiny::imageOutput()`, etc) are **fill item**s by default, so this behavior "just works" in those scenarios. And, in some of these situations, it's helpful to remove `card_body()`'s padding, which can be done via [spacing & alignment utility classes](#spacing-alignment).
:::
...
As you'll learn in [column-based layouts](layouts.html), `layout_column_wrap()` is great for multi-column layouts that are responsive and accommodate for [filling output](#filling-outputs). Here we have an equal-width 2-column layout using `width = 1/2`, but it's also possible to have [varying column widths](layouts.html#varying-widths).
...
`layout_column_wrap()` is especially nice for laying out multiple cards since each card in a particular row will have the same height (by default). Learn more in [column-based layouts](layouts.html).
...
An additional benefit that comes with using shiny is the ability to use `shiny::getCurrentOutputInfo()` to render new/different content when the output container becomes large enough, which is particularly useful with `card(full_screen = T, ...)`. For example, you may want additional captions/labels when a plot is large, additional controls on a table, etc (see the [value boxes](value-boxes.html) article for a clever use of this).
:::
