...
::: row
::: col-md-6
By default, a `card()`'s size grows to accommodate the size of it's contents. Thus, if a `card_body()` contains a large amount of text, tables, or other elements, you may want to specify a `height`. That said, when laying out [multiple cards](#multiple-cards), you probably shouldn't specify a `height` on the card-level. Instead, the height should probably come from a containing element, like `layout_column_wrap()`.
...
When a `card()` (or one of it's parent elements) is opinionated about it's height -- i.e., `height` is specified and/or `full_screen = TRUE` -- it's often convenient to allow re-sizable contents (e.g., plots, tables, etc) to grow and shrink as needed to fit inside the card body so the user doesn't need to scroll inside the card. This can be done by first using `card_body_fillable()` to create a fillable card body container and then placing "fill items" inside of the fillable container.
