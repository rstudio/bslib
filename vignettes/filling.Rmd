...
::: {.row .mt-3}
::: col-md-6
As a result, a common way in which (1) breaks down is that a [non-fill]{.b-orange} element, like a `div()`, comes between [fillable]{.b-pink} and [fill]{.b-blue}. In fact, you'll run into this exact behavior when using `uiOutput()` to insert a dynamically rendered [fill]{.b-blue} item into a [fillable container]{.b-pink}.
:::
...
In this section, we take what we learned about the theory of [fillable containers]{.b-pink} and [fill items]{.b-blue} and put it into practice.
...
Since `layout_column_wrap()` is also a [fill item]{.b-blue} (defaults to `fill = TRUE`), it will grow/shrink it a similar fashion. It also defaults to `fillable = TRUE`, which in this case, means each column gets wrapped in a [fillable container]{.b-pink} (and thus, `plots[[1]]` and `plots[[1]]` are allowed to grow/shrink to the `layout_column_wrap()` container).
