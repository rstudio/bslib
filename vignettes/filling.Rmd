...
`{bslib}` UI components (e.g., [cards](cards.html), [sidebars](sidebars), etc) are designed to accommodate filling layouts. In short, these components possess the potential to grow/shrink in size when they appear as direct children of a **fillable container** with an opinionated height (e.g., `page_fillable()`). 
...
card_plot_fill <- card(
  full_screen = TRUE,
  card_header("Diamond cut by clarity"),
  card_body(cut_clarity, class = "p-0")
)
...
basic_sidebar <- layout_sidebar(
  ...
  layout_column_wrap(
    ...
    card(
      ...
      card_body(price, class = "p-0")
    ),
    card(
      full_screen = TRUE,
      card_header("Diamond carat"),
      card_body(carat, class = "p-0")
    )
  ),
  br(),
  card(
