...
::: col-md-6
As eluded to in the [Carrying fill](#carrying-fill) section, `uiOutput()` puts an additional UI element around `renderUI()`'s return value. So, in order to carry the potential to fill down to a fill item (e.g., `plot_ly()`), mark `uiOutput()` as a fill carrier.
:::
