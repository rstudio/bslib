...
That being said, some of these formats still need attention to make them
more themeable (CSS may need porting to `{bslib}`-compatible Sass code).

### Does `{bslib}` work with `{shinydashboard}`?

No, we currently recommend [using the `{fresh}` package to accomplish
similar theming with
