# FROM SHINY PACKAGE, WITH MODIFICATIONS
  ...
  options <- mapply(
    ...
    FUN = function(value, name) {
      ...
      if (inline) {
        ...
        tags$div(
          class = type,
          tags$label(inputTag, tags$span(pd$html, pd$deps))
        )
      }
    },
    SIMPLIFY = FALSE,
...
# Ensures x has names, even if they're empty strings.
asNamed <- function(x) {
  if (is.null(names(x))) {
