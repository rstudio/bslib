    ...
    font_obj <- 
      switch(
        ...
        google = 
          sass::font_google(
            family = font$family,
            wght = brand_remap_font_weight(font$weight) %||% seq(100, 900, by = 100),
...
brand_font_bunny <- function(family, weight = NULL, style = NULL, display = NULL) {
  ...
  url_query <- paste0(names(params), "=", utils::URLencode(unlist(params)), collapse = "&")
